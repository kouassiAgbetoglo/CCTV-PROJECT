...
router.post('/changePw', authenticateToken, async (req, res) => {
    const { confirmPassword, currentPassword, newPassword } = req.body;
    const username = req.user;

    console.log(confirmPassword, currentPassword, newPassword, username)

    // Return if fields are missing
    if (!confirmPassword || !currentPassword || !newPassword || !username) {
